...
let enviando;
      ...
      if (Array.isArray(global.videoList2) && global.videoList2.length > 0) {
        ...
        if (matchingItem) {
          if (index < matchingItem.urls.length) {
            ...
            enviando = false  
            throw `*[❗] There was no link for that number, please enter a number between 01 and ${matchingitem.urls.length}*`; 
            }
        } else {
          enviando = false  
  ...
  try {
   ...
   if (fileSizeInMB > 100) {
    ...
    await conn.sendMessage(m.chat, {video: buff, caption: `*▢ Title:* ${ttl_1}\n*▢ Video size:* ${roundedFileSizeInMB} MB\n\n *─○* _Mr-Malik_ _by_ _Nasrullah_`, fileName: ttl_1 + '.mp4', mimetype: 'video/mp4'}, {quoted: m});
        enviando = false   
   }
 } catch (ee) {
    console.log(ee)
  try {
    ...
    await conn.sendMessage(m.chat, {video: {url: dl_url}, fileName: `${ttl}.mp4`, mimetype: 'video/mp4', caption: `*▢ Title:* ${ttl}\n*▢ Size:* ${size}\n\n *─○* _Mr-Malik_ _by_ _Nasrullah_`, thumbnail: await fetch(yt.thumbnail)}, {quoted: m});
        enviando = false
  } catch (ee2) {
    console.log(ee2)
    try {
      const mediaa = await ytMp4(youtubeLink);
      await conn.sendMessage(m.chat, {video: {url: mediaa.result}, fileName: `error.mp4`, caption: `\n\n *─○* _Mr-Malik_ _by_ _Nasrullah_`, thumbnail: mediaa.thumb, mimetype: 'video/mp4'}, {quoted: m});
           enviando = false
    } catch {
      try {
