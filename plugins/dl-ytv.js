...
const handler = async (m, {conn, args, usedPrefix, command}) => {
      ...
      if (Array.isArray(global.videoList) && global.videoList.length > 0) {
        ...
        if (matchingItem) {
          if (index < matchingItem.urls.length) {
            ...
            enviando = false  
            throw `*[❗] There was no link for that number, please enter a number between 1 and ${matchingitem.urls.length}*`; 
            }
        } else {
          enviando = false  
          throw `*[❗] To be able to use the command in this way(${usedPrefix + command} <numero>),  ${usedPrefix}playlist <texto>*`;
        }
      } else {
        enviando = false  
  ...
  const { key } = await m.reply(`*_⏳Please wait, Mr malik ᴘʀᴏᴄᴇssing ur ᴠɪᴅᴇᴏ...⏳_*\n\n*◉ ᴠɪᴅᴇᴏ  ᴄᴏᴍᴀɴᴅ #playdoc ᴏ #play.2 ᴏ #ytmp4doc ◉*`);
  try {
   ...
   if (fileSizeInMB > 100) {
    ...
    await conn.sendMessage(m.chat, {text: `*[ ✔ ] Video downloded and successfully sent.*\n\n*—◉ send in document format because the video length ${roundedFileSizeInMB} MB and exceeds the limit  by WhatsApp..*\n*◉ Tite:* ${ttl_1}`, edit: key}, {quoted: m});
    enviando = false
   } else {
    ...
    await conn.sendMessage(m.chat, {text: `*[ ✔ ] Video Downloaded.*`, edit: key}, {quoted: m});
    enviando = false   
   }
 } catch (ee) {
    console.log(ee)
  try {
    ...
    await conn.sendMessage(m.chat, {text: '*[ ✔ ] Video downloaded successfully.*', edit: key}, {quoted: m});
    enviando = false
  } catch (ee2) {
    console.log(ee2)
    try {
      const mediaa = await ytMp4(youtubeLink);
      await conn.sendMessage(m.chat, {video: {url: mediaa.result}, fileName: `error.mp4`, caption: `_Mr-Malik_`, thumbnail: mediaa.thumb, mimetype: 'video/mp4'}, {quoted: m});
      ...
      try {
        ...
        await conn.sendMessage(m.chat, {text: `*[ ❌ ] The video could not be downloaded or sent, try again.*`, edit: key}, {quoted: m});
