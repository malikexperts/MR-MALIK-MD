...
const handler = async (m, {conn, text, args, usedPrefix, command}) => {
  ...
  try {
    ...
    const desc1n = `*Hello! 🌟*\n *📽️ Here is your Tiktok video.*\n\n *Remember that if you want to convert the video into audio, you can do it easily by responding to the video with the command #tomp3 🎧* `;   await conn.sendMessage(m.chat, {video: dataFn.data, caption: desc1n}, {quoted: m});
  } catch (ee1) {
  try {
    //const aa = {quoted: m, userJid: conn.user.jid};
    //const prep = generateWAMessageFromContent(m.chat, {extendedTextMessage: {text: texto, contextInfo: {externalAdReply: {title: 'ᴛʜᴇ ᴍʏsᴛɪᴄ - ʙᴏᴛ', body: null, thumbnail: imagen1, sourceUrl: 'https://github.com/BrunoSobrino/TheMystic-Bot-MD'}, mentionedJid: [m.sender]}}}, aa);
    ...
    const desc1 = `*Hello! 🌟*\n *📽️ Here is your Tiktok video.*\n\n *Remember that if you want to convert the video into audio, you can do it easily by responding to the video with the command #tomp3 🎧* `;    await conn.sendMessage(m.chat, {video: {url: dataF.play}, caption: desc1}, {quoted: m});
  } catch (e1) {
    try {
      ...
      try {
        ...
        const te = `*Hello! 🌟*\n *📽️ Here is your Tiktok video.*\n\n *Remember that if you want to convert the video into audio, you can do it easily by responding to the video with the command #tomp3 🎧* `;   await conn.sendMessage(m.chat, {video: {url: p.nowm}, caption: te}, {quoted: m});
      } catch (e3) {
        try {
