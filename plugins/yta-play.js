...
const handler = async (m, {text, conn, args, usedPrefix, command}) => {
    ...
    if (index >= 0) {
      if (Array.isArray(global.videoList) && global.videoList.length > 0) {
        ...
        if (matchingItem) {
          ...
          throw `*[❗] Para poder hacer uso del comando de esta forma (${usedPrefix + command} <number>), example ${usedPrefix}y <text>*`;
        }
      } else {
  ...
  try {
   ...
   if (fileSizeInMB > 50) {
    ...
    await conn.sendMessage(m.chat, {text: `*✅ Audio Downloded Successfully.*`, edit: key}, {quoted: m});
    enviando = false   
   }    
  } catch {
    console.log('noooooo')
  try {
    ...
    await conn.sendMessage(m.chat, {text: '*✅Audio Downloaded successfully.*', edit: key}, {quoted: m});
  } catch {
    try {
...
handler.command = /^(audio|a|yt(a|mp3))$/i;
